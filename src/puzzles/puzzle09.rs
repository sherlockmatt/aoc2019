...
pub fn run(input: String) -> Vec<String> {
    ...
    IntcodeMachine::new(&mut program.clone(), &mut vec![1], &mut outputs, 0).execute_until_halt();

    answers.push(format!("{}", outputs[0]));

    outputs.clear();
    IntcodeMachine::new(&mut program.clone(), &mut vec![2], &mut outputs, 0).execute_until_halt();
