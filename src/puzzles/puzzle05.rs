...
pub fn run(input: String) -> Vec<String> {
    ...
    IntcodeMachine::new(&mut inputs.clone(), &mut vec![1], outputs, 0, 0).execute_until_halt();

    answers.push(format!("{}", outputs.iter().last().unwrap()));

    outputs.clear();
    IntcodeMachine::new(&mut inputs.clone(), &mut vec![5], outputs, 0, 0).execute_until_halt();
